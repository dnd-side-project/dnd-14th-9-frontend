...
export function MyTimer({ sessionId, sessionDurationMinutes }: MyTimerProps) {
  const maxSeconds = sessionDurationMinutes * 60;

  const { formatted, isFocusing, isRunning, focusRate, progress, start, pause } = useSessionTimer({
    sessionId,
    maxSeconds,
    autoStart: true,
  });
