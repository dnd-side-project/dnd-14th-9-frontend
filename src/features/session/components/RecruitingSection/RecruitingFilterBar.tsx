...
// TODO(장근호): 실제 API 스펙에 맞춰 옵션 값 조정 필요
const TIME_SLOT_OPTIONS: FilterOption[] = [
  { value: "MORNING", label: "오전 (06:00 ~ 12:00)" },
  { value: "AFTERNOON", label: "오후 (12:00 ~ 18:00)" },
  { value: "EVENING", label: "저녁 (18:00 ~ 00:00)" },
];
...
export function RecruitingFilterBar({ sort, onSortChange }: RecruitingFilterBarProps) {
  ...
  const timeSlot = searchParams.get("timeSlots");
  ...
  const timeSlotLabel = getOptionLabel(TIME_SLOT_OPTIONS, timeSlot, "시작 시간대");
    ...
    <div className="gap-md flex flex-col">
      ...
      <div className="flex items-center gap-[15px]">
        ...
        <Filter
          size="large"
          radius="sm"
          onClick={() => updateFilter("timeSlots", getNextOptionValue(TIME_SLOT_OPTIONS, timeSlot))}
          className={cn("w-auto shrink-0", timeSlot && "text-text-primary")}
        >
          {timeSlotLabel}
        </Filter>
