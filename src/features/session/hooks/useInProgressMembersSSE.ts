...
const SSE_EVENT_NAME = "in-progress-members-updated";
...
export function useInProgressMembersSSE({
  ...
  const [data, setData] = useState<InProgressEventData | null>(null);
  ...
  const getSSEUrl = useCallback(() => {
    return `/api/sse/in-progress/${sessionId}`;
  }, [sessionId]);
  ...
  useEffect(() => {
    ...
    // 이벤트 리스너 등록
    const unsubscribeEvent = client.on<InProgressEventData>(SSE_EVENT_NAME, (eventData) => {
      setData(eventData);
      setError(null);
    });
