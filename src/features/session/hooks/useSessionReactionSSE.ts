...
const SSE_EVENT_NAME = "reaction-updated";
...
export function useSessionReactionSSE({
  ...
  const [data, setData] = useState<SessionReactionEventData | null>(null);
  ...
  const getSSEUrl = useCallback(() => {
    return `/api/sse/reaction/${sessionId}`;
  }, [sessionId]);
  ...
  useEffect(() => {
    ...
    // 이벤트 리스너 등록
    const unsubscribeEvent = client.on<SessionReactionEventData>(SSE_EVENT_NAME, (eventData) => {
      setData(eventData);
      setError(null);
    });
